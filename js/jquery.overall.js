...
$(document).ready(function(){
	...
	// 定义所有单选和复选的宽度为auto
	$(':checkbox').css({'width':'auto','vertical-align':'middle','display':'inline'});
	$(':radio').css({'width':'auto','vertical-align':'middle','display':'inline'});
	// 为表格的偶数行添加隔行变色的背景   
	$('.dbList').not($('.not')).find('tbody').find('tr:odd').css('background-color','#f9f9f9');
...
$(document).ready(function(){
	// 选项卡最后一个li加右边框
	$('.tab').find('.tab-label').find('li').each(function(){
        var obj=$(this);
        if(obj.attr('untab')!='true'){
            obj.append('<span class="tab_left"></span><span class="tab_right"></span>');
        }
    });
});